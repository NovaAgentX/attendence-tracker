...
const API_URL = "https://script.google.com/macros/s/AKfycbw0oHqg1cUfZobMelP84mkQ2LSzSFubl37ROjyHqA67xRB4aAGbP7UKhsXPvHqlVRqlpA/exec";
...
async function apiCall(action, payload = {}, token = null) {
  ...
  // NOTE: this uses GET with query-string parameters rather than POST with a
  // JSON body. Google Apps Script Web Apps are known to sometimes omit the
  // Access-Control-Allow-Origin header on POST responses (causing a browser
  // "Failed to fetch" / CORS error), while GET responses from the same /exec
  // endpoint reliably include it. Code.gs routes GET query-string actions
  // through the exact same handler as POST, so behavior is identical.
  if (isConfiguredUrl()) {
    try {
...
async function initDashboardPage() {
  ...
  if (navSignOutBtn) {
    navSignOutBtn.addEventListener("click", async (e) => {
      e.preventDefault();
      // Record the attendance logout in the sheet before clearing the
      // session - mirrors the dedicated "Logout Now" button so any sign-out
      // path always fills in Logout Date/Time/Status.
      try {
  ...
  // 4. Load My Leaves (balance summary + history) - read-only, admin-managed
  loadMyLeaves();

  async function loadMyLeaves() {
    const leavesTbody = document.getElementById("employeeLeavesTbody");
    const emptyLeavesBox = document.getElementById("emptyLeavesBox");
    const balanceCardsEl = document.getElementById("leaveBalanceCards");

    try {
      const res = await apiCall("getMyLeaves");
      if (res && res.success) {
        if (balanceCardsEl && res.balance) {
          balanceCardsEl.innerHTML = res.balance.map(b => {
            const isUnlimited = b.quota === null;
            return `
              <div style="flex:1;min-width:130px;background:var(--bg-subtle,#f8fafc);border:1px solid var(--border-color,#e2e8f0);border-radius:10px;padding:12px 14px;">
                <div style="font-size:11px;font-weight:700;color:var(--text-muted);text-transform:uppercase;letter-spacing:0.03em;margin-bottom:4px;">${escapeHtml(b.type)}</div>
                <div style="font-size:20px;font-weight:800;color:var(--text-heading);">${isUnlimited ? b.used : b.remaining}</div>
                <div style="font-size:11px;color:var(--text-muted);">${isUnlimited ? "days taken (no quota)" : `remaining of ${b.quota}`}</div>
              </div>
            `;
          }).join("");
        }
        ...
        if (res.leaves && res.leaves.length > 0) {
          if (emptyLeavesBox) emptyLeavesBox.style.display = "none";
          if (leavesTbody) {
            leavesTbody.innerHTML = "";
            res.leaves.forEach(lv => {
              const tr = document.createElement("tr");
              tr.innerHTML = `
                <td><strong>${escapeHtml(lv.leaveType)}</strong></td>
                <td>${formatDisplayDate(lv.fromDate)}</td>
                <td>${formatDisplayDate(lv.toDate)}</td>
                <td>${lv.days}</td>
                <td>${lv.reason ? escapeHtml(lv.reason) : '<span style="color:var(--text-muted)">-</span>'}</td>
              `;
              leavesTbody.appendChild(tr);
            });
          }
        } else {
          if (leavesTbody) leavesTbody.innerHTML = "";
          if (emptyLeavesBox) emptyLeavesBox.style.display = "block";
        }
      }
    } catch (err) {
...
async function initAdminPage() {
  ...
  tabButtons.forEach(btn => {
    btn.addEventListener("click", (e) => {
      ...
      if (targetTab === "leaves") loadLeaves();
  ...
  if (adminLogoutBtn) {
    adminLogoutBtn.addEventListener("click", async () => {
      ...
      try {
        // Record the actual attendance logout (Logout Date/Time/Status in the
        // sheet) before clearing the session - previously this button only
        // cleared local session data and never told the backend, so the
        // Attendance row stayed "Logged In" forever with blank logout fields.
        await apiCall("logout");
      } catch (e) {
        // Even if this fails (e.g. offline), still sign the user out locally
        // so they aren't stuck - but log it for visibility.
  ...
  async function loadEmployees() {
    ...
    try {
      ...
      if (res && res.success) {
        currentEmployeesList = res.employees || [];
        renderAdminEmployeeRows(currentEmployeesList, adminEmployeesTbody);
        populateLeaveEmployeeDropdowns(currentEmployeesList);
      } else {
  ...
  // Fill both the "Add Leave" employee select and the leave list filter
  // dropdown with the current active employee directory (includes admins,
  // so an admin can log leave for themselves too).
  function populateLeaveEmployeeDropdowns(employees) {
    const selectEl = document.getElementById("leaveEmployeeSelect");
    const filterEl = document.getElementById("leaveEmployeeFilter");
    const options = employees
      .filter(e => e.active)
      .map(e => `<option value="${escapeHtml(e.employeeId)}">${escapeHtml(e.employeeName)} (${escapeHtml(e.employeeId)})</option>`)
      .join("");

    if (selectEl) {
      selectEl.innerHTML = `<option value="">Select employee&hellip;</option>` + options;
    }
    if (filterEl) {
      const currentVal = filterEl.value;
      filterEl.innerHTML = `<option value="">All Employees</option>` + options;
  ...
  const adminLeavesTbody = document.getElementById("adminLeavesTbody");
  ...
  async function loadLeaves() {
    if (adminLeavesTbody) {
      adminLeavesTbody.innerHTML = `<tr><td colspan="8" style="text-align:center;padding:24px;"><span class="spinner spinner-primary"></span> Loading leave records...</td></tr>`;
    }
    try {
      const payload = leaveEmployeeFilter && leaveEmployeeFilter.value ? { employeeId: leaveEmployeeFilter.value } : {};
      const res = await apiCall("getLeaves", payload);
      if (res && res.success) {
        currentLeavesList = res.leaves || [];
        renderAdminLeaveRows(currentLeavesList, adminLeavesTbody);
      } else {
        showToast(res.error || "Failed to load leave records", "error");
      }
    } catch (e) {
  ...
  function renderAdminLeaveRows(leaves, tbody) {
    if (!tbody) return;
    if (!leaves || leaves.length === 0) {
      tbody.innerHTML = `<tr><td colspan="8" style="text-align:center;padding:32px;color:var(--text-muted);">No leave records found.</td></tr>`;
      return;
    }

    const typeColors = {
      "LOP": "badge-inactive",
      "Sick Leave": "badge-role-employee",
      "Casual Leave": "badge-role-admin",
      "Vacation Leave": "badge-active",
      "Wellness Leave": "badge-logged-in"
    };

    tbody.innerHTML = "";
    leaves.forEach(lv => {
      const tr = document.createElement("tr");
      const badgeClass = typeColors[lv.leaveType] || "badge-inactive";
      tr.innerHTML = `
        <td><strong>${escapeHtml(lv.employeeName)}</strong><br><span style="font-size:11px;color:var(--text-muted);">${escapeHtml(lv.employeeId)}</span></td>
        <td><span class="badge ${badgeClass}">${escapeHtml(lv.leaveType)}</span></td>
        <td>${formatDisplayDate(lv.fromDate)}</td>
        <td>${formatDisplayDate(lv.toDate)}</td>
        <td>${lv.days}</td>
        <td style="max-width:220px;white-space:normal;">${lv.reason ? escapeHtml(lv.reason) : '<span style="color:var(--text-muted);">-</span>'}</td>
        <td>${escapeHtml(lv.addedBy)}</td>
        <td><button class="btn btn-outline btn-sm delete-leave-btn" data-id="${escapeHtml(lv.leaveId)}">Delete</button></td>
      `;
      tbody.appendChild(tr);
    });
    ...
    tbody.querySelectorAll(".delete-leave-btn").forEach(btn => {
      btn.addEventListener("click", async () => {
        ...
        try {
          const res = await apiCall("deleteLeave", { leaveId });
          if (res && res.success) {
            showToast(res.message || "Leave entry deleted", "success");
            loadLeaves();
          } else {
            showToast(res.error || "Failed to delete leave entry", "error");
          }
        } catch (e) {
          showToast("Error deleting leave entry", "error");
        }
      });
    });
  }

  if (leaveEmployeeFilter) leaveEmployeeFilter.addEventListener("change", loadLeaves);
  if (refreshLeavesBtn) refreshLeavesBtn.addEventListener("click", loadLeaves);

  // Add Leave Modal wiring
  const addLeaveModal = document.getElementById("addLeaveModal");
  const openAddLeaveModalBtn = document.getElementById("openAddLeaveModalBtn");
  const closeAddLeaveModalBtn = document.getElementById("closeAddLeaveModalBtn");
  const addLeaveForm = document.getElementById("addLeaveForm");

  if (openAddLeaveModalBtn) {
    openAddLeaveModalBtn.addEventListener("click", () => {
      if (addLeaveForm) addLeaveForm.reset();
      if (addLeaveModal) addLeaveModal.classList.add("open");
    });
  }
  if (closeAddLeaveModalBtn) {
    closeAddLeaveModalBtn.addEventListener("click", () => {
  ...
  if (addLeaveForm) {
    addLeaveForm.addEventListener("submit", async (e) => {
      e.preventDefault();
      const submitBtn = document.getElementById("submitAddLeaveBtn");
      ...
      const toDate = document.getElementById("leaveToDate").value || fromDate;
      const reason = document.getElementById("leaveReason").value;

      if (!employeeId || !leaveType || !fromDate) {
        showToast("Please fill in Employee, Leave Type, and From Date", "error");
        return;
      }

      if (submitBtn) submitBtn.disabled = true;
      try {
        const res = await apiCall("addLeave", { employeeId, leaveType, fromDate, toDate, reason });
        if (res && res.success) {
          showToast(res.message || "Leave recorded successfully", "success");
          if (addLeaveModal) addLeaveModal.classList.remove("open");
          if (addLeaveForm) addLeaveForm.reset();
          loadLeaves();
        } else {
          showToast(res.error || "Failed to add leave", "error");
        }
      } catch (err) {
        showToast("Error adding leave: " + err.message, "error");
      } finally {
